...
docker exec firebox-client bash -c "mkdir -p $CONTAINER_SYNC_DIR/parent_folder/child_folder/grandchild_folder"
if [ $? -eq 0 ]; then
...
docker exec firebox-client bash -c "echo 'Parent folder file' > $CONTAINER_SYNC_DIR/parent_folder/parent_file.txt"
docker exec firebox-client bash -c "echo 'Child folder file' > $CONTAINER_SYNC_DIR/parent_folder/child_folder/child_file.txt"
docker exec firebox-client bash -c "echo 'Grandchild folder file' > $CONTAINER_SYNC_DIR/parent_folder/child_folder/grandchild_folder/grandchild_file.txt"
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT folder_name, folder_path FROM folders WHERE folder_name='parent_folder';" | grep -q "parent_folder"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT folder_name, folder_path FROM folders WHERE folder_name='child_folder';" | grep -q "child_folder"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT folder_name, folder_path FROM folders WHERE folder_name='grandchild_folder';" | grep -q "grandchild_folder"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT file_name, file_path FROM files_metadata WHERE file_name='parent_file.txt';" | grep -q "parent_file.txt"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT file_name, file_path FROM files_metadata WHERE file_name='child_file.txt';" | grep -q "child_file.txt"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT file_name, file_path FROM files_metadata WHERE file_name='grandchild_file.txt';" | grep -q "grandchild_file.txt"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT f1.folder_name as folder, f2.folder_name as parent FROM folders f1 LEFT JOIN folders f2 ON f1.parent_folder_id = f2.folder_id ORDER BY f2.folder_name, f1.folder_name;"

# Step 7: Create a file in a deeply nested folder
echo -e "\n${YELLOW}Step 7: Creating a new file in a deeply nested folder...${NC}"
docker exec firebox-client bash -c "mkdir -p $CONTAINER_SYNC_DIR/deep/nested/folder/structure"
docker exec firebox-client bash -c "echo 'Deeply nested file' > $CONTAINER_SYNC_DIR/deep/nested/folder/structure/deep_file.txt"
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT folder_name FROM folders WHERE folder_name='structure';" | grep -q "structure"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT file_name FROM files_metadata WHERE file_name='deep_file.txt';" | grep -q "deep_file.txt"
if [ $? -eq 0 ]; then
...
docker exec firebox-client sqlite3 /app/data/firebox.db "SELECT folder_id, folder_name, folder_path, parent_folder_id FROM folders;"
