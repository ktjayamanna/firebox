...
echo -e "${BLUE}=========================================${NC}"
echo -e "${GREEN}Dropbox File Modification Smoke Test${NC}"
echo -e "${BLUE}=========================================${NC}"
...
docker exec dropbox-client bash -c "echo 'Original content' > $TEST_FILE"
if [ $? -eq 0 ]; then
...
ORIGINAL_HASH=$(docker exec dropbox-client sqlite3 /app/data/dropbox.db "SELECT file_hash FROM files_metadata WHERE file_name='modification_test.txt';")
echo -e "Original file hash: $ORIGINAL_HASH"

# Step 4: Modify the file
echo -e "\n${YELLOW}Step 4: Modifying the file...${NC}"
docker exec dropbox-client bash -c "echo 'Modified content' > $TEST_FILE"
if [ $? -eq 0 ]; then
...
NEW_HASH=$(docker exec dropbox-client sqlite3 /app/data/dropbox.db "SELECT file_hash FROM files_metadata WHERE file_name='modification_test.txt';")
...
FILE_ID=$(docker exec dropbox-client sqlite3 /app/data/dropbox.db "SELECT file_id FROM files_metadata WHERE file_name='modification_test.txt';")
