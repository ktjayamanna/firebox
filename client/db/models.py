from sqlalchemy import Column, Integer, PrimaryKeyConstraint, String, ForeignKey, DateTime, Text
...
class FilesMetaData(Base):
    ...
    def __repr__(self):
        return f"<FilesMetaData(file_id='{self.file_id}', folder_id='{self.folder_id}', file_type='{self.file_type}')>"
...
class Chunks(Base):
    ...
    last_synced = Column(DateTime, default=lambda: datetime.now(datetime.timezone.utc))
